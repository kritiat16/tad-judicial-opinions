...
But applying the validation framework directly turns out to surface a fundamental structural difference between the two domains. To see why, it helps to clarify what a circuit split is and why it serves as a natural ground truth signal for legal disagreement.

The United States federal court system is organized into thirteen regional appellate courts called "circuits," each with binding authority only within its own geographic territory. The Ninth Circuit covers most of the West Coast; the Second Circuit covers New York and parts of the Northeast; and so on. When a novel legal question arises -- say, whether a hyperlink in an employer email is sufficient notice to start the 90-day clock on an employment discrimination lawsuit -- different circuits can, and frequently do, reach different conclusions. The First Circuit might say the hyperlink notice is inadequate; the Eighth Circuit might say it is adequate. Both opinions are precedential, both are reasoned at length, both bind the lower courts in their respective territories. Until the Supreme Court intervenes (which happens for only a small fraction of splits), federal law on the same question literally differs depending on where one lives.

This is what legal scholars call a **circuit split**: two or more federal appellate courts reaching opposite conclusions on the same legal question. For our purposes, a circuit split is an ideal ground truth signal for a specific kind of relationship between two documents -- two opinions that (a) address the same legal question, (b) draw on the same body of statutory and constitutional text, (c) cite a heavily overlapping set of prior cases, and (d) arrive at opposite conclusions. The Congressional Research Service catalogues these splits annually for Congress, naming the controlling cases on each side. We use these catalogues as our source.

The patent paper's ground truth (interferences) is, by contrast, a *topic identity* signal: two patents are "interfering" when they describe the same invention. Cosine similarity in any reasonable embedding space should track this well; "describes the same thing" and "embeds to a similar vector" are operationalizations of the same underlying construct. Circuit splits are structurally different. Circuit-split opinions discuss the same legal question, cite the same precedents, and use the same doctrinal vocabulary. What distinguishes them is not what they are about, but where they land. Text similarity, as conventionally implemented, is not designed to detect this.
...
We aggregate these quantities across splits and report four summary statistics. The within-split A↔B mean is the average over 28 pairs. The random baseline mean covers 1,512 cross-split pairs. The topic-confound gap is defined as (random mean) - (within-split mean); under Framework B, a positive gap indicates the model is not fooled by topic similarity (random pairs score no lower than within-split pairs), while a negative gap indicates the model is fooled (within-split pairs score higher than random). The discrimination rate is the fraction of splits where the within-split A↔B similarity falls below the median of Side A's similarity to all other opinions; higher values indicate better stance discrimination, with 0.5 corresponding to chance.

# Results

Table 2 reports the validation results.

| Model | Within-split A↔B | Random baseline | Topic-confound gap | PR AUC | Disc. rate | MWU U | MWU p |
|---|---|---|---|---|---|---|---|
| TF-IDF | 0.125 | 0.086 | -0.039 | 0.151 | 0.500 | 24227 | 0.556 |
| Legal-BERT | 0.913 | 0.913 | +0.0001 | 0.066 | 0.464 | 23933 | 0.637 |
| GTE | 0.832 | 0.826 | -0.007 | 0.089 | 0.536 | 24581 | 0.465 |

*Gap = random baseline - within-split mean. Positive values indicate the model is not fooled by topic (random pairs score no lower than within-split pairs) -- the desired behavior under Framework B. Negative values indicate topic-vocabulary confounding (within-split pairs rated higher than random). PR AUC chance level = 0.017 (28 positive pairs out of 1,540 total). MWU p-values from two-sided Mann-Whitney U test comparing within-split A↔B similarity scores against cross-split similarity scores. p > 0.05 indicates no statistically significant difference.*
...
All three models are at or near zero on the topic-confound gap under Framework B. Legal-BERT's gap is essentially zero (+0.0001), the best result under Framework B: this model scores within-split pairs no higher than random pairs, indicating it is not fooled by topic vocabulary overlap. GTE's gap is slightly negative (-0.007), indicating mild topic confounding -- within-split pairs score marginally above the random baseline. TF-IDF's gap is the most negative (-0.039), with within-split pairs scored 45% higher than random pairs on average; this is the worst result under Framework B and the expected behavior for a bag-of-words model that has no representation of meaning beyond word identity.
...
PR AUC provides a complementary view. Chance level is 0.017 (28 positive pairs out of 1,540 total). All three models exceed chance, with TF-IDF achieving the highest PR AUC (0.151), consistent with its strong topic-vocabulary signal inflating within-split rankings under Framework A. Legal-BERT achieves the lowest PR AUC (0.066), consistent with its near-zero gap: a model that does not over-rate within-split pairs will also not rank them particularly high. The two metrics together confirm the central finding: TF-IDF is the most topic-confounded model; Legal-BERT is the least.

To formally test whether within-split similarity scores are statistically distinguishable from cross-split scores, we apply a two-sided Mann-Whitney U test for each model. All three models return p-values far above the conventional 0.05 threshold (Legal-BERT p=0.637, GTE p=0.465, TF-IDF p=0.556), confirming that no model produces a distribution of within-split similarity scores that is statistically separable from the random baseline. This formalizes the near-chance finding reported above.

A final observation concerns the two framings introduced in the Validation task subsection. Under Framework A -- the patent-paper-aligned framing in which a good model assigns higher within-split similarity than random baseline -- TF-IDF performs best, with within-split similarity 45% above random. Legal-BERT performs essentially identically across the two pair classes, and GTE shows a small (0.8%) within-split lift. This ordering, however, is the opposite of what Framework B prefers: under Framework B, a near-zero or negative gap is undesirable when driven by topic confounding, and a positive gap (random >= within-split) is the desired behavior. By that measure, Legal-BERT (+0.0001) is the best performer; GTE's small negative gap (-0.007) puts it intermediate; and TF-IDF (-0.039) is the worst. The two framings yield opposite verdicts on the same numbers, and that opposition is itself a methodological observation: the choice of evaluation framing is, like the choice of model, a substantive decision rather than a technical detail. This mirrors the broader argument of Ganguli et al. (2024) about model selection, one level up.
...
[1] Beim, D., & Rader, K. (2019). Legal Uniformity in the American Courts: A Study of Intercircuit Splits. *Journal of Empirical Legal Studies*, 16(2), 305--336.

[2] Chalkidis, I., Fergadiotis, M., Malakasiotis, P., Aletras, N., & Androutsopoulos, I. (2020). LEGAL-BERT: The Muppets straight out of Law School. *Findings of EMNLP 2020*, 2898--2904.

[3] Congressional Research Service (2024). *The U.S. Courts of Appeals: Background and Circuit Splits from 2023*. CRS Report R47899.

[4] Congressional Research Service (2025). *The U.S. Courts of Appeals: Background and Circuit Splits from 2024*. CRS Report R48369.

[5] Congressional Research Service (2026). *The U.S. Courts of Appeals: Background and Circuit Splits from 2025*. CRS Report R48846.

[6] Ethayarajh, K. (2019). How Contextual are Contextualized Word Representations? Comparing the Geometry of BERT, ELMo, and GPT-2 Embeddings. *Proceedings of EMNLP-IJCNLP 2019*, 55--65.

[7] Ganguli, I., Lin, J., Meursault, V., & Reynolds, N. (2024). *Patent Text and Long-Run Innovation Dynamics: The Critical Role of Model Selection*. NBER Working Paper 32934.

[8] Hellman, A. (2008). The Law of the Circuit Revisited: What Role for Majority Rule? *Southern Illinois University Law Journal*, 32, 625--652.

[9] Holzenberger, N., Blair-Stanek, A., & Van Durme, B. (2020). A Dataset for Statutory Reasoning in Tax Law Entailment and Question Answering. *Proceedings of the Natural Legal Language Processing Workshop 2020*, 31--38.

[10] Li, Z., Zhang, X., Wang, Y., Long, D., Xie, P., & Zhang, M. (2023). Towards General Text Embeddings with Multi-stage Contrastive Learning. arXiv:2308.03281.

[11] Locke, D., & Zuccon, G. (2018). A Test Collection for Evaluating Legal Case Law Search. *SIGIR 2018*, 1261--1264.

[12] Sadeghian, A., Sundaram, L., Wang, D. Z., Hamilton, W. F., Branting, K., & Pfeifer, C. (2018). Automatic Semantic Edge Labeling over Legal Citation Graphs. *Artificial Intelligence and Law*, 26, 127--144.
