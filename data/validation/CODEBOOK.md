...
| `side_a_cl_id` | int | CourtListener opinion ID for the Side A opinion, used by `fetch_courtlistener.py` to pull full text via the REST API. | `10384512` | Integer stored as a bare number (no quotes). Used to construct the API endpoint `https://www.courtlistener.com/api/rest/v4/opinions/{id}/`. All 30 rows are populated. |
...
| `side_b_cl_id` | int | CourtListener opinion ID for the Side B opinion. | `10023613` | Same conventions as `side_a_cl_id`. All 30 rows are populated. |
